...
async def init_file(file_name_data: str, size):
    ...
    # Путь до сжатого
    path_compress = os.path.join('uploads', ".".join(
        file_name_data[:-1]), ".".join(file_name_data[-1])+".webp")
    ...
    for level, count in detector_of_details(size):
        ...
        convert_file(path_upload, path_compress)
...
def convert_img_to_webp(png_file_path: str, webp_file_path: str, quality: int):
    ...
    with Image.open(png_file_path) as img:
        ...
        print("start confert")
        # quality можно настроить от 0 до 100
